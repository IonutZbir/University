...
\begin{lemma}
    ...
    \begin{equation}\label{eq:2}
        \mathbf{x}^{(0)} := \mathbf{x} \qquad \mathbf{x}^{(t+1)} := P\mathbf{x}^{(t)} = PP\mathbf{x}^{(t-1)} = \dots = P^t\mathbf{x} 
    \end{equation}
    \begin{proof}
...
\textbf{Osservazione:} La matrice $P$ è \textbf{diagonalizzabile}, poichè è una matrice \textit{simile} ad una simmetrica [Da dimostrare quando avrò voglia e tempo]. 
...
\]
quindi, abbiamo ottenuto che il nostro sistema convergo a qualcosa (anche quando $G$ non è regolare).
...
\begin{teorema}\label{teo:terminazione}
    ...
    \]
    \begin{proof}
        Poiché consideriamo il valore \textit{locale} di ciacun nodo, ci interessa utilizzare la $\norm{\cdot}_{\infty}$ in quanto ci dò un upper bound \textit{massimale} al valore dei nodi. Per ogni $t \leq 1$, abbiamo che
...
\begin{teorema}[Terminazione e Corretezza (Accuratezza)]
    ...
    \begin{proof}
        ...
        \begin{align*}
            ...
            &= \lambda_1^{t}\alpha_1\mathbf{u}_1 + \dots + \lambda_1^{t}\alpha_n\mathbf{u}_n - \alpha_1\mathbf{u}_1 \\
            [\lambda_1 = 1] &= (\alpha_1\mathbf{u}_1 - \alpha_1\mathbf{u}_1) + \lambda_2^{t}\alpha_2\mathbf{u}_2 + \dots + \lambda_1^{t}\alpha_n\mathbf{u}_n \\
            &=  \lambda_2^{t}\alpha_2\mathbf{u}_2 + \dots + \lambda_1^{t}\alpha_n\mathbf{u}_n \\
...
\paragraph{Il ruolo dei Grafi Expander.} 
In un grafo expander, il secondo autovalore $\lambda$ è limitato superiormente da una costante strettamente minore di 1, indipendentemente dal numero di nodi $n$. Di conseguenza, il termine $\log(1/\lambda)$ al denominatore della formula del tempo di convergenza rimane costante, rendendo il suo impatto trascurabile sulle prestazioni asintotiche. Questo garantisce che il protocollo raggiunga il consenso in tempo puramente logaritmico rispetto alla dimensione della rete. Tale proprietà riflette l'elevata conduttanza del sistema e l'assenza di colli di bottiglia topologici (sparse cuts) che rallenterebbero la diffusione della massa.