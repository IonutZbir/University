...
\begin{teorema}[Message Complexity]
    ...
    \begin{proof}
        Consideriamo i nodi $u, v \in V$ tali che l'arco $(u, v) \in E$. Ad ogni round $t \geq 1$, $u$ invia $x^{(t)}(u)$ a $v$ e viceversa, per un totale di 2 messaggi. Quindi, il numero totale di messaggi in un round è:
        \[
            M[\texttt{AVG}]_t = \sum_{v \in V} 2\delta(v) = 2\sum_{v \in V} \delta(v) = 4m \in \theta(m)
        \]
    \end{proof}
\end{teorema}
...
Per trovare una base $B$ ortonormale, in particolare l'autovettore $\mathbf{u}_1$, si prende un autovettore generico $\mathbf{v}_1$ relativo a $\lambda = 1$ e lo si normalizza, quindi
\[
...
dove
\[
    \mathbf{v}_1 = D^{-1}\langle \frac{\delta_1}{2m}, \frac{\delta_2}{2m}, \dots, \frac{\delta_n}{2m} \rangle = \langle \frac{1}{2m}, \frac{2}{2m}, \dots, \frac{n}{2m} \rangle
\]
...
\begin{proof}
    Sappiamo che il sistema converge a $\alpha_1\mathbf{u}_1$. Essendo una base ortonormale, $\alpha_1 = \langle \mathbf{x}, \mathbf{u_1}$, quindi
    \[
    ...
    \begin{itemize}
        ...
        \[
            \norm{\mathbf{v_1}}_2^2 = n \left( \frac{\delta}{2m} \right)^2 = \frac{nd^2}{4m^2}
        \]
