...
app.get("/items-complete", (req, res) => {
	const items = data.filter((item) => item.completato === true) // ricerca di un evento
	if(items === undefined){ // verifica esistenza dell'oggetto cercato
		res.status(404).json({
			"status": "fail",
			"data": {
				"items": "No Items Completed Found"
			}
		})
	}else{ // tutto apposto, restuisco l'oggetto desiderato
		res.status(200).json({
